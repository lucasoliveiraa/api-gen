...
describe('CategoryService', () => {
  ...
  it('should return list category', async () => {
    ...
    expect(categories).toEqual([new ReturnCategory(categoryMock)]);
  });
  ...
  it('should return delete result in success', async () => {
    const deleteResult = await service.delete(categoryMock.id);
  ...
  it('should send relations in request findOne', async () => {
    ...
    await service.delete(categoryMock.id);
  ...
  it('should return error if category with relations', async () => {
    ...
    expect(service.delete(categoryMock.id)).rejects.toThrowError(
      BadRequestException,
    );
  });
  ...
  it('should return category in update category', async () => {
    const spy = jest.spyOn(categoryRepository, 'findOne');
    const category = await service.update(categoryMock.id, updateCategoryMock);
